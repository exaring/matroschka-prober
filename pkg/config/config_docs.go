...
func init() {
	...
	ConfigDoc.Fields[1].Description = "Socket to use for exposing metrics. Takes a string with the format <ip_address>:<port>.\nFor IPv6, the string must have the format [<address>]:port."
	ConfigDoc.Fields[1].Comments[encoder.LineComment] = "Socket to use for exposing metrics. Takes a string with the format <ip_address>:<port>."
